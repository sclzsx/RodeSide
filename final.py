...
def print_img_info(img, gain=1, verbose=0, save=0):
    print(np.min(img), np.max(img), np.mean(img), img.shape, img.dtype)
    if verbose:
        print(img)
    img = (img * gain).astype('uint8')
    if save:
...
def filt_forground_with_area_thresh(edge, area_thresh = 1000):
    edge1 = (edge * 255).astype('uint8')
    edge2 = np.zeros_like(edge1)
    _,contours,_=cv2.findContours(edge1,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
...
if __name__ == '__main__':
    ...
    # print_img_info(hue_mask, gain=255, save=1)

    crop_mask = gen_crop_mask(gray)
    # print_img_info(crop_mask, gain=255, save=1)
    ...
    edge = filt_forground_with_area_thresh(edge)
    # print_img_info(edge, gain=255, save=1)

    skeleton = morphology.skeletonize(edge)
    # print_img_info(skeleton, gain=255, save=1)

    left_mask, right_mask = gen_left_right_mask(skeleton)

    edge_left = edge * left_mask
    edge_left = filt_forground_with_area_thresh(edge_left)
    # print_img_info(edge_left, gain=255, save=1)

    edge_right = edge * right_mask
    edge_right = filt_forground_with_area_thresh(edge_right)
    # print_img_info(edge_right, gain=255, save=1)
    ...
    # plt.plot(left_X, left_Y2)
    # plt.plot(right_X, right_Y)
    # plt.plot(right_X, right_Y2)
    # plt.show()
    ...
    cv2.imwrite('Data2/' + img_name + '2.png', bgr)
